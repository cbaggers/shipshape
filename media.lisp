...
(defun copy-media (path manifest)
  (cond ((cl-fad:directory-exists-p path)
         (copy-dir path manifest))
        ((cl-fad:file-exists-p path)
         (copy-file path manifest))
...
(defun copy-file (path manifest)
  (with-slots (system system-media-path) manifest
    (let* ((rel-dir (pathname-directory (local-path "" system)))
           (rel-len (length rel-dir))
           (src (local-path path system))
           (src-dir (pathname-directory src)))
      (assert (equal (subseq src-dir 0 rel-len) rel-dir))
      (let* ((rel-path (make-pathname
                        :directory (cons :relative (subseq src-dir rel-len))
                        :name (pathname-name src)
                        :type (pathname-type src)))
             (dst (merge-pathnames rel-path
