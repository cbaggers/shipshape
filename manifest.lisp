...
(defun check-macro-args (args)
  (destructuring-bind (profile build-path binary-name c-library-path
    ...
    (assert (or (null compression)
		(and (numberp compression)
		     (>= compression -1)
		     (<= compression 9))))
...
(defun parse-macro-args (system args)
  (let* ((last-kwd-pos (position-if #'keywordp args :from-end t))
         (paths-pos (if last-kwd-pos
                        (+ last-kwd-pos 2)
                        0))
         (key-args (subseq args 0 paths-pos))
         (paths (subseq args paths-pos)))
